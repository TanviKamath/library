...
function createTextTexture(gl, text, font = 'bold 30px Figtree', color = '#2b1a0a') {
  ...
  // Set padding for the golden badge
  const padX = 24;
  const padY = 12;
  canvas.width = textWidth + padX * 2;
  canvas.height = textHeight + padY * 2;
  
  // Draw the gold gradient/metallic background
  const gradient = context.createLinearGradient(0, 0, canvas.width, canvas.height);
  gradient.addColorStop(0, '#ebd8b0');
  gradient.addColorStop(0.5, '#c5a66d');
  gradient.addColorStop(1, '#b19052');
  ...
  // Border
  context.strokeStyle = '#826532';
  context.lineWidth = 2.5;
  context.stroke();
  
  // Text
  let fillCol = color;
  if (fillCol && fillCol.startsWith('var(')) {
    fillCol = '#2b1a0a';
  }
  context.font = font;
  ...
  createShader() {
    ...
    img.onerror = () => {
      ...
      if (ctx) {
        ...
        ctx.fillText(this.text || 'Book', 200, 300);
      }
      texture.image = canvas;
...
export default function CircularGallery({
  ...
  }, [items, bend, textColor, borderRadius, font, fontUrl, scrollSpeed, scrollEase, onSelectCard]);
  return (
    <div
